...
### 圖表元件
- 營收與訂單合併圖（combo chart）：原生 SVG，同一張圖以長條呈現營收、以折線＋節點呈現訂單數，兩者共用 X 軸（依時間範圍動態分組），採左右雙 Y 軸刻度概念
...
### 統一訂單格式（一筆訂單含多項商品）
每筆訂單為單一物件，商品放在 `items` 陣列中，使一單多列合併為一筆訂單。`amount` 為訂單層級金額（不隨商品項數重複）。
...
```javascript
{
  ...
  logistics: "宅配",
  amount: 1893,          // 訂單總額（訂單層級，只計一次）
  items: [
    { product: "極光透亮二合一洗卸蜜 150mL", quantity: 1 },
    { product: "栗米光采修護精華 30mL", quantity: 1 }
  ]
}
```

### 平台原始欄位對照
各平台匯出檔欄位對照至統一格式（依實際匯出檔欄位名）：

| 統一欄位 | Pinkoi 欄位 | CYBERBIZ 欄位 |
|----------|-------------|---------------|
| orderId | 訂單編號 | 訂單編號（如 #1230） |
| date | 訂購日期 | 時間 |
| recipient | 收件人姓名 | 收件人名稱 |
| phone | 收件人電話 | 收件人電話 |
| address | 收件人地址 | 收件人地址 |
| items[].product | 商品名稱 | 商品名稱（+ 商品款式） |
| items[].quantity | 數量 | 數量 |
| logistics | 寄送方式 | 出貨方式 |
| amount | 總金額 | 總額（訂單層級） |

### CYBERBIZ 一單多列向下填補（fill-down）
CYBERBIZ 匯出時，一張訂單的多項商品會拆成多列：只有第一列（主列）有完整客戶與金額資訊，後續列（明細列）僅有商品名稱與數量、其餘欄位為空。

清洗規則：
1. 逐列讀取，若該列有訂單編號且收件人非空 → 視為主列，建立新訂單並加入第一項商品。
2. 若該列收件人為空（或訂單編號與前一列相同）→ 視為明細列，將其商品追加到前一筆訂單的 `items`。
3. `amount` 僅取主列的「總額」，避免明細列重複加總。

### 模擬資料
- Pinkoi 約 20 筆 + CYBERBIZ 約 15 筆，改為 items 格式，寫在 app.js 頂部
- 涵蓋不同商品、出貨方式、日期與月份（供時間範圍營收分析）
- 使用假造個資（姓名、電話、地址），僅保留真實的商品與出貨方式樣貌
...
### Property 1: 訂單合併正確性
CYBERBIZ 一單多列經向下填補後，同一訂單編號合併為單一訂單物件，所有商品皆進入該訂單的 items 陣列，不遺漏商品也不產生重複訂單。
...
### Property 3: 營收加總正確性
營收以訂單層級 amount 加總，一筆訂單只計一次金額（不因 items 多商品而重複）。依時間範圍與平台篩選後各分組加總總和應等於該範圍內的總營收。
